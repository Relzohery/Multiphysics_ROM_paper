...
The effort described extends previous work\cite{elzohery2021exploring} aimed to develop and assess a Reduced-order model (ROM) framework that can be used for different purposes, such as design optimization and propagating input parameter uncertainties like nuclear data.
Here, the target applications is non-linear.
...
Where applications of the ROM to the diffusion equation have been introduced before \cite{sartori2014comparison, sartori2014reduced, tsujita2021fast}, this work focuses on treating the non-linearity induced by coupling with different physics.
Previous work has used DEIM to approximate non-linear functions resulting from multiphysics coupling such as the temperature and the velocity field \cite{german2019application, german2022gen}.
Here, the MDEIM is used to approximate the projection of the problem operator that otherwise must be computed at each non-linear iteration within each time step, which increases the ROM cost.
...
For the parameterized ROM, the maximum relative error in the assembly power was $1\times 10^{-3}$\%, 
...
It employs two energy groups and two delayed precursor groups and represents a 2-D quarter-core, BWR model subject to a control-rod ejection with adiabatic heating \cite{anl}. The two groups data are given in Table~\ref{tab:two group data}, where $\text{B}^2$ is the axial buckling for both energy groups as given in the benchmark specification.
