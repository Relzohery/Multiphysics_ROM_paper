...
We agree with reviewer that the DEIM was used before and we have acknowledged their work in the introduction and cited their paper.
However, it is important to clarify that in this work, we used a different variant of the DEIM algorithm, which it is matrix version.
Moreover, where DEIM we used previously to approximate non-linear function such as the temperature or the flow filed, we have used MDEIM to approximate the problem operator in case an affine decomposition is not readily obtainable. And to the author knowledge the Matrix version of DEIM was not used before in the nuclear community.
